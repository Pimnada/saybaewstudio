...
เมื่อได้ไฟล์ `.svg` มาแล้ว ติดตั้งด้วย `php tools/install-icons.php โฟลเดอร์`
ดูรายละเอียดท้ายเอกสาร — ทั้งเว็บจะเปลี่ยนตามทันทีทุกหน้า
...
## หลังจากได้ไฟล์มาแล้ว — ติดตั้งด้วยคำสั่งเดียว

เซฟไฟล์ `.svg` ไว้ในโฟลเดอร์ไหนก็ได้ **ตั้งชื่อไฟล์ให้ตรงกับชื่อไอคอน**
(`camera.svg` → ไอคอน `camera`) แล้วรัน

```bash
php tools/install-icons.php --dry-run ~/Downloads/โฟลเดอร์ไอคอน
```

ดูว่าจะเปลี่ยนอะไรบ้าง ถ้าถูกต้องค่อยรันโดยไม่ใส่ `--dry-run` (ใส่หลายโฟลเดอร์พร้อมกันได้)

ตัวติดตั้งจัดการให้เองทั้งหมด

- ถอดแท็ก `<svg>` ออก เหลือแต่ตัวรูป
- ถอด `style="color:#xxxxxx"` ที่โปรแกรมออกแบบมักแถมมา — **สำคัญมาก** ถ้าปล่อยไว้
  สีไอคอนจะถูกล็อกตายและหายไปในโหมดกลางคืน
- ถอด attribute ที่ซ้ำกับที่ตัวห่อกำหนดไว้แล้ว แต่เก็บของที่ต่างไว้ครบ
  (เช่นจุดทึบในไอคอนลายเส้น)
- เดาเองว่าเป็นไอคอนลายเส้นหรือทึบ
- ตรวจ syntax ก่อนเขียนทับ ถ้าผลลัพธ์ไม่ผ่านจะไม่แตะไฟล์เดิมเลย

**ห้ามเปลี่ยนชื่อไอคอน** — ชื่อถูกอ้างถึงทั่วทั้งเว็บ ถ้าตั้งชื่อไฟล์ไม่ตรงกับที่มีอยู่
ตัวติดตั้งจะข้ามและบอกให้ทราบ ไม่เขียนมั่ว

จากนั้นเปิดเว็บดูสักหน้า ถ้าตัวไหนเพี้ยนจะเห็นทันทีเพราะทุกหน้าใช้ชุดเดียวกัน
